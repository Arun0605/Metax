...
const DigitalTwinAssessment = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    has_osa: false, ahi_severity: '', snoring: false, tired: false, observed_apnea: false, neck_cm: '',
    ...
    has_oa: false, pain_severity: '', daily_nsaid: false,
  ...
  const handleDownloadPDF = () => {
    ...
    const opt = {
      margin:       0.3,
      filename:     `MetaX_Clinical_Twin_${formData.patient_name.replace(/\s+/g, '_')}.pdf`,
            ...
            {step === 1 && (
              <div className="space-y-6 animate-in fade-in">
                <div className="grid grid-cols-2 gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="col-span-2"><label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Patient Name</label><input type="text" className="w-full p-3 bg-white rounded-lg border" value={formData.patient_name} onChange={(e)=>setFormData({...formData, patient_name:e.target.value})} /></div>
                  <div><label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Age</label><input type="number" className="w-full p-3 bg-white rounded-lg border" value={formData.age} onChange={(e)=>setFormData({...formData, age:e.target.value})} /></div>
                  <div><label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Gender</label><select className="w-full p-3 bg-white rounded-lg border" value={formData.gender} onChange={(e)=>setFormData({...formData, gender:e.target.value})}><option value="female">Female</option><option value="male">Male</option></select></div>
                  <div><label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Height (cm)</label><input type="number" className="w-full p-3 bg-white rounded-lg border" value={formData.height} onChange={(e)=>setFormData({...formData, height:e.target.value})} /></div>
                  <div><label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Weight (kg)</label><input type="number" className="w-full p-3 bg-white rounded-lg border" value={formData.weight} onChange={(e)=>setFormData({...formData, weight:e.target.value})} /></div>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in slide-in-from-right">
                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                    <h3 className="font-bold text-slate-800 text-sm uppercase mb-4">Diabetes & Liver</h3>
                ...
                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                    <h3 className="font-bold text-slate-800 text-sm uppercase mb-4">Lipid Profile</h3>
        ...
        <div className="animate-in fade-in duration-1000 bg-white">
          ...
          <div ref={reportRef} className="p-10 md:p-14 bg-white relative print:p-6 print:m-0">
            ...
            <div className="flex justify-between items-end border-b-4 border-slate-900 pb-6 mb-10">
              <div><h1 className="text-4xl font-black text-slate-900 uppercase">MetaX Clinical Twin</h1><p className="text-indigo-600 font-bold tracking-widest uppercase text-[10px] mt-1">Predictive Analytics Platform</p></div>
              <div className="text-right p-4 bg-slate-50 rounded-xl border border-slate-100">
                <p className="text-lg font-black text-slate-900 uppercase">{formData.patient_name}</p>
                <p className="text-xs font-bold text-slate-400 mt-1">MRN: {formData.patient_id} • PROG: {result.procedure_modeled}</p>
              </div>
            </div>
          ...
          {/* ON-SCREEN CONTROLS */}
          <div className="flex justify-center gap-4 py-10 print:hidden">
            <button onClick={()=>setStep(1)} className="px-8 py-3 bg-slate-100 rounded-xl font-bold uppercase text-xs tracking-wider transition hover:bg-slate-200">Restart</button>
            ...
            <button onClick={handleDownloadPDF} className="px-10 py-3 bg-slate-900 text-white rounded-xl font-bold uppercase text-xs tracking-wider shadow-lg hover:bg-slate-800 transition">Download Evidence PDF</button>
          </div>
        </div>
      )}

      {/* LUXURIOUS LIGHT-THEMED INTERACTIVE LIFESTYLE MODAL */}
      {showLifestyleModal && result && (
